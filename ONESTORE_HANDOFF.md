...
**서버 (1~6단계 완료 + 인터널 배포 완료 — 2026-09-02)**

인터널(`iap-internal-api.9c.gg`)에 `git-03ad60b1` 로 배포됐고 배선까지 확인했다:

    시크릿    iap-env 에 onestore-client-id/-client-secret/-host (version 7ce103c4)
    차트      9c-infra#3592 (API_ONESTORE_* optional env)
    이미지    9c-infra#3593
    alembic   9c1e2f3a4b5c → a5f3c8d21b7e (store enum 에 ONESTORE 확인)
    파드에서  토큰 발급 성공(36자), 구매 조회가 원스토어 앱까지 도달(NoSuchData)

즉 **서버는 실기기 결제를 받을 준비가 됐다.** 남은 건 실기기 검증과 클라이언트 `_store` 정리,
그리고 아래 "안 한 것" 두 가지다.

⚠️ **인터널 PG 는 15.12, 메인넷은 10.15 다.** 마이그레이션이 `autocommit_block()` 을 쓰는
근거는 PG 10 제약(`ALTER TYPE ADD VALUE` 를 트랜잭션 안에서 못 돌린다)이고 15 에는 불필요하다.
"인터널에서 됐으니 메인넷도 된다" 는 판단이 성립하지 않는 자리다 — 메인넷은 별도로 확인할 것.
...
- **`/api/validate` 엔드포인트** — 손대지 않았다. **애초에 앱에 등록되지 않는 라우터다**
  ...
  더 생긴다.
- **원스토어 환불 회수** — `track_google_refund` 대응물이 없다. `voucher_reconcile_task` 의
  ...
  **해법은 폴링이 아니라 PNS(Push Notification Service)다.** 원스토어가 결제·결제취소 발생 시
  ...
  가능) 라고 경고한다 — **지급은 지금처럼 서버 API 조회로 하고, PNS 는 환불 감지 트리거로만**
  쓰는 게 맞는 사용법이다. 받은 취소 알림으로 `enqueue_revoke_for_receipt` 를 부르면
  회수 경로는 store 무관이라 그대로 붙는다.
  영수증 `data` 에 purchaseToken 이 남아 있어 폴링(재조회)도 가능하지만 PNS 가 있으면 그쪽이 낫다.
...
**샌드박스 실측 결과 (2026-09-02, 인터널 인증정보로 확인)**

인터널 시크릿을 넣은 뒤 검증환경(`sbpp.onestore.net`)에 직접 쳐 봤다. 통과:

- **인증정보 유효** — `POST /v7/oauth/token` 성공, AccessToken 36자(문서 명세와 일치).
- **경로 정합** — 존재하지 않는 토큰으로 구매 조회 시 원스토어 JSON 오류
  `{"error":{"code":"NoSuchData"}}` 가 온다. 즉 URL 조립과 오류 파싱이 실물과 맞는다.
- **인코딩 필요성 확인** — 경로 세그먼트에 `%2B` `%3D` `%25` `%2E%2E` 는 전부 앱까지
  도달한다(`%2E%2E` 가 `..` 로 정규화되지 않는 것도 확인). 인코딩은 필요하고 옳다.

**🔴 원인 확정 (2026-09-02, 실제 샌드박스 구매로)**: 조회가 계속 `NoSuchData` 였던 것은
`x-market-code` 누락이었다. `MKT_GLB` 를 붙이면 200 과 정확한 구매 정보가 온다(§4 참조).
서버에 반영했다.

발견해서 고친 것:

- 🔴 **`%2F` 는 원스토어 프론트 서버가 HTML 404 로 거부한다**(Apache `AllowEncodedSlashes
  Off` 로 보인다). 즉 `/` 가 든 purchaseToken 은 **원스토어 자신의 엔드포인트로도 조회가
  안 된다.** 그 HTML 404 를 "구매 없음" 으로 오해하면 안 되므로 부르기 전에 끊는다
...
x-market-code: MKT_GLB          ← 🔴 없으면 한국 마켓에서 조회한다
```

- `clientId` 가 **예전의 `packageName` 자리**다 (2025-03-20 개편). 앱 식별자로 쓴다.

**🔴 `x-market-code` — 문서는 선택, 글로벌 배포엔 사실상 필수**

문서에 "마켓 구분 코드가 없는 경우에는 원스토어(기본값)로 호출됩니다" 라고만 적혀 있어 놓치기
쉽다. 우리 앱은 배포국가가 미국, 테스트 계정이 싱가포르 — **전부 글로벌 마켓**이라 헤더가 없으면
원스토어가 한국 마켓에서 찾고 거기엔 그 구매가 없다. 2026-09-02 실측(실제 샌드박스 구매로):

    헤더 없음    404 NoSuchData
    MKT_ONE      404 NoSuchData
    MKT_GLB      200 구매 정보 반환 ✅

즉 이 헤더가 빠지면 **모든 구매가 "없는 구매"로 보인다.** 한국 배포만 하면 실제로 선택이다.
설정으로 뺐다: `API_ONESTORE_MARKET_CODE`(기본 `MKT_GLB`). 실패 메시지에 `(market=…)` 을
같이 남기므로, 전 구매가 NoSuchData 면 로그에서 마켓을 먼저 볼 것.

⚠️ **마켓에 따라 응답의 시각 기준이 다르다** — `MKT_ONE`=UTC+09, `MKT_GLB`=UTC+00.
지금 쓰는 `purchaseTime` 은 epoch 밀리초라 영향이 없지만, 다른 시각 필드를 쓰게 되면 반드시
확인할 것(잘못 잡으면 9시간 어긋난다).

**acknowledge 도 같은 헤더가 필요하다.** 다른 마켓으로 승인하면 안 먹고, 그러면 3일 뒤
자동환불된다(지급은 이미 나간 상태로).
...
**~~배포국가 ↔ 테스트 계정 국가 불일치~~ — 해소됨 (2026-09-02)**

앱 배포국가는 미국, 테스트 계정은 싱가포르인데 **미국 계정을 새로 만들 필요가 없었다.** 문서:

> "상품을 배포하고자 하는 OS에서 반드시 한 번은 샌드박스 테스트를 진행해야 합니다."
> "**국가/지역 별 테스트는 필요하지 않습니다.**"

싱가포르 계정으로 한 테스트로 충분하다. 진짜 문제는 국가가 아니라 **서버가 `x-market-code` 를
안 보내서 한국 마켓을 조회한 것**이었다(§4).

**테스트 ID 는 결제환경을 하나만 가진다**

> "하나의 테스트 ID는 하나의 결제환경만 선택가능하며, 결제환경 설정은 언제든 변경할 수 있습니다."
> "테스트 환경을 변경했다면, **반드시 앱을 종료하고 다시 시작**해야 합니다."

나중에 상용테스트로 넘어갈 때 필요하다.
