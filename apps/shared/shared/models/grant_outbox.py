...
class GrantOutbox(AutoIdMixin, TimeStampMixin, Base):
    ...
    **"온체인에 넣었나?" 아웃박스 마커**다.
    ...
    대신 `external_ref` 의 앞부분(`shop:`)이 출처 단서다. ⚠️ **강제되지는 않는다** — 예전엔
    허용 네임스페이스 등록제가 있었지만 제거됐다(근거는 app/grant_guard.py 도커스트링).
    즉 지금은 계약이 아니라 **관례**이고, 감사 로그의 출처도 그만큼만 믿을 수 있다.
    """
    ...
    __table_args__ = (
        ...
        # ⚠️ 아래 둘은 **지금 아무 쿼리도 쓰지 않는다.** 시간창 상한·아바타 축 카운트를
        #   위해 넣었는데 그 가드들이 제거됐다(app/grant_guard.py 도커스트링). 쓰기 증폭만
        #   남으므로 드롭 리비전이 후속이다 — 운영 조회(기간·아바타별 지급 이력)에 여전히
        #   쓸모가 있어 이 커밋에서 같이 지우지는 않는다.
