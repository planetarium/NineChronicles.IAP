...
from shared.consts import CURRENCY_LIST
...
class GQL:
    def __init__(self, url: str, jwt_secret: str = None):
        ...
        transport = RequestsHTTPTransport(
            url=self._url, verify=True, retries=2, headers=self.__create_header()
        )
    ...
    def _transfer_asset(self, pubkey: bytes, nonce: int, **kwargs) -> bytes:
        ...
        if not (sender and recipient and currency and amount):
            raise ValueError(
                "All params must be exist to execute this action: sender, recipient, currency, amount"
            )
        if currency not in CURRENCY_LIST:
            raise ValueError(
                f"Given currency {currency} it not valid. Please input one of {CURRENCY_LIST}"
            )
        if float(amount) <= 0:
            raise ValueError(
                f"Given amount {amount} is not positive. Please give positive value"
            )
    ...
    def create_action(
        self, action_type: str, pubkey: bytes, nonce: int, **kwargs
    ) -> bytes:
